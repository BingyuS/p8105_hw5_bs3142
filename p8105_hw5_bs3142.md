...
**Description** The dataset consists of homicide information in 50 large U.S. cities, each row is a case showing victim information (name, race, age, sex), murder data, location information (city, state, latitude, longitude), and disposition information of the case. There are a total of **52179** cases.
...
prop_test_results = 
  cases_nest %>%
  mutate(statistics = map2(.x = cases_nest$n_unsolved, .y = cases_nest$n_total, ~ prop_test(x = .x, n = .y))) %>% #a new variable for test statistics applies to each sub-dataframes
